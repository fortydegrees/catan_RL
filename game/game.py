...
class Game(object):
    ...
    def reset(self):
        self.board.reset()
        self.player_order = [PlayerId.Blue, PlayerId.Red]
    ...
    def apply_action(self, action, return_message=True):
        ...
        if action["type"] == ActionTypes.PlaceSettlement:
            ...
            if edge is None:
                ...
                if self.initial_placement_phase:
                    ...
                    if first_settlements_placed < 2:
                        self.update_players_go()
                    elif first_settlements_placed == 2 and second_settlements_placed == 0:
                        pass
                    elif first_settlements_placed == 2 and second_settlements_placed < 2:
            ...
            #print(f"{player.id} stealing from {player_id}")
        ...
        if action["type"] != ActionTypes.EndTurn and action["type"] != ActionTypes.DiscardResource:
    ...
    def update_longest_road(self, player_id):
        ...
        if self.longest_road["player"] == player_id:
            if self.longest_road["count"] > max_path_len:
                ...
                for other_pid in [PlayerId.Blue, PlayerId.Red]:
    ...
    def update_player_resource_estimates(self, resources, player_updating, player_who_stole = None):
        ...
        for player in [ PlayerId.Red, PlayerId.Blue]:
    ...
    def update_resource_estimates_monopoly(self, player_id, res, num_res):
        ...
        for player in [PlayerId.Blue, PlayerId.Red]:
            if player == player_id:
                ...
                for o_player in [ PlayerId.Blue, PlayerId.Red]:
                    if o_player == player:
                        continue
